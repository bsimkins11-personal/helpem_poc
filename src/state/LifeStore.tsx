...
const createSeedData = () => {
  ...
  return { seedAppointments, seedTodos, seedHabits };
};

// Create seed data once
const { seedAppointments, seedTodos, seedHabits } = createSeedData();
...
export function LifeProvider({ children }: LifeProviderProps) {
  ...
  const value = useMemo<LifeContextType>(() => ({
    ...
    logHabit,
    addAppointment,
  }), [todos, habits, appointments, addTodo, completeTodo, updateTodoPriority, addHabit, logHabit, addAppointment]);
